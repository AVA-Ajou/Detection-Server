...
#
# 앱(`BackendClassificationClient.signalOf`)과 **같은 값이어야 한다.** 어긋나면 앱은 피싱으로
# 보는데 서버는 단계를 안 주는(또는 그 반대) 상태가 되고, 화면에 나가는 유일한 판정 표시가
# 단계라서 그대로 사용자에게 보인다.
#
# 70 → 50 → 33 으로 내려왔다. 지금 이 값은 "피싱이라고 부르는 선"이 아니라 **단계를 계산해
# 볼 만한 최소선**이다. 앱이 두 문턱으로 판정하기 때문이다.
#
#   위험도 66 이상          앱이 경보(빨강). 단계는 있으면 붙이고 없어도 경보다
#   위험도 33 이상 66 미만   앱이 단계 유무로 가른다 — 있으면 주의보(주황), 없으면 표시 안 함
#
# 그래서 33 아래로는 계산할 이유가 없고, 33 위로는 **앱이 판단할 재료를 서버가 쥐고 있으면
# 안 된다.** 이 값을 앱의 `CAUTION_THRESHOLD` 보다 높이면 주의보가 영영 나오지 않는다.
ASSESS_THRESHOLD = 33
