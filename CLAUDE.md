...
- **단계를 손대면 `stage_eval.py` 를 돌릴 것 — 통화와 문자 둘 다.** 정규식은 고치기 쉬워서
  눈대중으로 바꾸기 쉬운데, 그러면 "나아 보인다"에서 멈춘다. 정답지는 규칙보다 **먼저**
  만들어졌다(`../Voice-Detection/eval/stage_gold.jsonl`, `stage_gold_sms.jsonl`) — 규칙에
  맞춰 정답을 고치지 말 것. `signals.py` 아래쪽의 문자 문형(링크·인증번호·앱 설치·상품권)은
  통화 36건 점수가 그대로(33/36)인 것을 확인하고 넣었다.
...
- **어댑터를 갱신하면 학습 저장소와 양쪽 다 바꿀 것.** 가중치를 여기에도 커밋하므로
  (clone 하면 바로 뜨도록) 같은 파일이 두 곳에 있다. 한쪽만 갱신하면 어느 것이 최신인지
  알 방법이 없고, 옛 가중치에 새 온도가 물리면 **에러 없이 확률만 틀어진다.**
  같은 파일인지는 해시로 확인한다 — `shasum -a 256 <양쪽 adapter_model.safetensors>`.
...
│                           #   python3 -m src.stage_eval --sms     # 문자 정답지 24건
├── src/sms_eval.py         # 문자 어댑터 기준선. 서버에 걸어 낱개 40건 + 다채널 시나리오 7건
│                           #   python3 -m src.sms_eval [--task sms2]   # 후보 어댑터 비교
│
├── adapters/               # 가중치까지 커밋한다 — clone 하면 바로 뜬다
