...
def _analyze_sms(text: str) -> dict:
    """SMS·카카오톡 텍스트를 sms 어댑터(LoRA)로 판정한다.

    학습이 완료된 adapter-sms 를 engine 이 로드하며, task="sms" 로 라우팅된다.
    단계 판정·위험도 보정은 engine.analyze() 내부에서 처리되므로 여기서 따로 하지 않는다.
    """
    if engine is None:
        raise HTTPException(503, "모델을 아직 올리는 중입니다.")
    result = engine.analyze("sms", text)
    _log(text, result)
    return result
