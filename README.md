...
**둘을 같이 보고** 다섯 등급(정상 · 예보 · 주의보 · 경보우려 · 경보)을 정한다 — 80 이상은
단계와 무관하게 경보, 50~80은 단계가 있으면 경보 없으면 경보우려, 20~50은 단계가 잡히면
주의보 아니면 예보다. **단계 번호 자체는 앱 화면에 나가지 않는다** — 등급을 가르는 데까지만
쓴다.

`ASSESS_THRESHOLD`(20)는 앱의 `CAUTION_THRESHOLD`와 같은 값이어야 한다. 한쪽만 옮기면
그 사이 구간이 단계를 못 받아 등급이 통째로 내려앉는다.
